...
class Permissions
{
	...
	public function __construct()
	{
		...
		if(!empty($_SESSION['user_name']) && ($_SESSION['user_logged_in'] == 1))
		{
			...
			{
				// Prompts for admin status modification
				if(isset($_POST['admin']))
				{
					$this->modifyAdminStatusConfirm($_POST['admin']);
				}
				else
				{
					$this->messages[] = "Can't remove all the admins!";
				}
	...
	/**
	 * Searches database and returns only data of all user for display
	 * This data is primarily for admin display
	 * Note:  this is data that can be displayed in an admin setting, but it will still not display password hashes
	 */
	public function getEachUsersData()
	{
		if($this->databaseConnection()) {
			// Queries all user data that is not "sensitive"
			$query = "SELECT `user_id`, `user_name`, `user_email`, `user_registration_datetime`, `admin` FROM `users`;";
			$query_user_info = $this->db_connection->prepare($query);
			$query_user_info->execute();

			// Returns all user data as object for display
			return $query_user_info->fetchAll(PDO::FETCH_OBJ);
		} else {
			// return false if database could not be connected to
			return false;
		}
	}
	...
	public function modifyAdminStatusConfirm($admin_confirm_array)
	{
		...
		if($this->databaseConnection())
		{
			...
			// variable holds information that a modify_admin (action) has been selected (for display of confirm button in html perhaps)
	...
	public function resetAccountConfirm($user_array)
	{
		...
		// variable holds information that an reset_account (action) has been selected (for display of confirm button in html perhaps)
		$this->confirm_action_prompt = true;
