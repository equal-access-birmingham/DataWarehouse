...
class Login
{
					...
					if ($query_update->rowCount()) {
						...
						if($result_row->user_password_change == 0) {
							...
							if($query_verify_account->rowCount()) {
								// Allows detection of password change and database update
								return true;
							} else {
								// The password has been changed, but the database has not registered it
								$this->errors[] = MESSAGE_DATABASE_ERROR;
							}
						} else {
							// Allows the function to return true even if this is not the user's first password change but the database was still updated with a password change
							return true;
						}
					} else {
	...
	/**
	 * Checks to see that user's account is valid for login based on a password change
	 */
	private function verifyAccount($user_name)
	{
		...
		if($result_row->user_password_change == 0) {
			...
			// User has changed password and accound it valid
			$this->user_is_verified = true;
		}
		// Default return as long as user_name is filled
		return true;
	}
