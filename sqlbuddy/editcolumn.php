...
if (isset($_POST['editParts']) && $conn->isResultSet($structureSql)) {
	...
	while ($structureRow = $conn->fetchAssoc($structureSql)) {
		if (in_array($structureRow['Field'], $editParts)) {
			...
			<td class="inputarea">
			<input type="text" class="text" class="form-control" name="NAME"<?php if ($firstField) echo ' id="EDITCOLUMNFIRSTFIELD"'; ?> value="<?php echo $structureRow['Field']; ?>" style="width: 125px" />
			</td>
			<td class="secondaryheader">
			<?php echo __("Type:"); ?>
			</td>
			...
			<td class="inputarea">
			<input type="text" class="text" class="form-control" name="VALUES" value="<?php if (substr($curextra, 0, 1) == "(" && substr($curextra, -1) == ")") echo $curextra; ?>" style="width: 125px" />
			</td>
			<td colspan="2">
			</td>
			</tr>
			...
			<td class="inputarea">
			<input type="text" class="text" class="form-control" name="SIZE" value="<?php echo $cursize; ?>" style="width: 125px" />
			</td>
			<td class="secondaryheader">
			<?php echo __("Default:"); ?>
			</td>
			<td class="inputarea">
			<input type="text" class="text" class="form-control" name="DEFAULT" value="<?php echo $structureRow['Default']; ?>" style="width: 125px" />
			</td>
			</tr>
			<?php
			...
			<input type="submit" class="btn btn-success" value="<?php echo __("Submit"); ?>" />&nbsp;&nbsp;<a onclick="cancelEdit('editform<?php echo $counter; ?>')"><?php echo __("Cancel"); ?></a>
			</td>
			</tr>
			</table>
			</div>
